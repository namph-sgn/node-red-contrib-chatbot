...
const { isCommand } = require('./regexps');
...
const utils = {
  ...
  /**
   * @method params
   * Return a getter function for params inside a message. Usage params = params(msg) then
   * const myParam = params('my_param', defaultValue)
   * @param {Object} msg
   * @return {Function}
   */
  params(msg) {
  ...
  /**
   * @method isCommand
   * Test if a message is a command message /my_command (use this and not the one in regexps)
   * @param {Object} msg
   * @return {Boolean}
   */
  isCommand(msg) {
    if (msg != null && msg.payload != null) {
      return isCommand(msg.payload.content);
    }
    return false;
  },
  ...
  /**
   * getChatbotId
   * Extract the chabotId from an incoming message
   * @param {Object} msg
   * @returns {String}
   */
  getChatbotId: msg => {
    return msg != null && msg.originalMessage != null ? msg.originalMessage.chatbotId : null;
  },
  ...
  getChatId: function(msg) {
    if (msg.originalMessage != null && msg.originalMessage.chatId != null) {
      return msg.originalMessage.chatId;
    } else if (_.isObject(msg.payload) && msg.payload.chatId != null) {
   ...
   * @return {Object}
   */
  appendPayload: function(send, msg) {
    return payload => {
      let combinedPayload = payload;
      if (validators.arrayOfMessage(msg.payload)) {
        // if array, then append
        combinedPayload =  [...msg.payload, payload];
      } else if (_.isObject(msg.payload) && !_.isEmpty(msg.payload.type) && msg.payload.inbound === false) {
        combinedPayload = [msg.payload, payload];
      }
      combinedPayload.inbound = false;

      send({ ...msg, payload: combinedPayload });
    }
  },
  ...
  /**
   * isSimulator
   * Test if incoming message is from simulator, don't rely on transport, could be impersonated user
   * @param {Object} msg
   * @return {Boolean}
   */
  isSimulator(msg) {
