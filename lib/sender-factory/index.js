...
const GenericBotNode = (
  ...
  return function(n) {
    ...
    if (!startNode) {
      ...
      console.log(warn(`${_s.capitalize(platform)} Bot ` + this.botname + ' will NOT be launched, environment is ' + environment));
      return;
    }
    // eslint-disable-next-line no-console
    console.log(green(`${_s.capitalize(platform)} Bot ` + this.botname + ' will be launched, environment is ' + environment));
    ...
    if (platformConfigs[node.botname] != null) {
      ...
      if (validation != null) {
        ...
        console.log('');
        console.log(lcd.error(`Found a ${_s.capitalize(platform)} configuration in settings.js "` + node.botname + '", but it\'s invalid.'));
        ...
        console.log('');
        console.log(lcd.grey(`Found a valid ${_s.capitalize(platform)} configuration in settings.js: "` + node.botname + '":'));
        console.log(prettyjson.render(platformConfigs[node.botname]));
        console.log('');
    ...
    if (botConfiguration.enableMissionControl) {
      // eslint-disable-next-line no-console
      console.log(lcd.warn('Using Mission Control context provider (SQLite) for ' + node.botname + '.'));
      botConfiguration.contextProvider = 'sqlite';
      botConfiguration.contextParams = {
        dbPath: RED.settings.missionControl.dbPath
      };
...
const GenericInNode = (platform, RED) => {
  ...
  return function(config) {
    ...
    const environment = global.environment === 'production' ? 'production' : 'development';
...
const GenericOutNode = (platform, RED) => {
  ...
  return function(config) {
    ...
    this.on('input', async function(message, send, done) {
      ...
      // finally send out
      try {
        await node.chat.send(message);
        if (node.config.storeMessages) {
          await storeOutboundMessage(message, node); // TODO remove node
        }
        ...
        // send payload to ws
        if (node.config.inspectMessages) {
          sendMessage('message.out', message.sentMessage);
        }
        done();
      } catch(error) {
